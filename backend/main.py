...
# CORS configuration to allow access from frontend
app.add_middleware(
   CORSMiddleware,
   allow_origins=["http://localhost:5173", "https://sports-ai-nine.vercel.app"],
   allow_credentials=True,
...
async def get_predictions():
    ...
    pregame_data = doc_ref.get()
    if not pregame_data.exists:
        raise HTTPException(status_code=404, detail="No pregame available")
    games = pregame_data.to_dict().get('games', [])
    return PredictionResponse(games=games)  # Return in the format of PredictionResponse
